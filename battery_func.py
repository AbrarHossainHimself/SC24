...
battery_data = pd.read_csv('data/test_data.csv').values

# Extract features and target without using DataFrame methods
X = battery_data[:, :6]
# Adjust the last column of X as needed, just like before
X[:, 5] = 1 - np.sum(X[:, :5], axis=1)

# If Y is supposed to be a single target variable column following the features
# Y = battery_data[:, 5].reshape(-1, 1)  # Ensuring Y is a 2D array for a single target variable
...
# For the sampler_dict, adjust according to the correct shape of Y
sampler_dict = {
    'X': X,
    'Y': Y,  # Directly use Y without trying to access a second column
    'model_hyperparams': model.current_hyperparams(),
    'biases': sampler.biases.clone(),
    'thetas': sampler.thetas.clone(),
    'weights': sampler.weights.clone(),
    'Phi': sampler.Phi.clone(),
